...
class ChartController extends Controller {
        ...
        do {
            try {
                ...
                if ($reverseFlag == 0) {
                    ...
                    abort('403', "Can not get this API data. From: {$fromTime}, Time Range: {$timeRange}, Type: {$type}. Error: {$e->getMessage()}");
                }
            }
        } while ($reverseFlag >= 1);
    }
    ...
    protected function getCurrentData($type, $interval, $utc) {
        ...
        do {
            try {
                $response = $client->request(
                    'GET',
                    'v3/instruments/' . $type . '/candles?granularity=' . $interval . '&count=1',
                    [
                        'headers' => $headers
                    ]
                );
                ...
                if ($reverseFlag == 0) {
                    ...
                    abort("Can not get this API data {$type}");
                }
            }
        } while ($reverseFlag >= 1);
    }
    ...
    protected function storeAPI($data, $timeRange) {
        ...
        ChartData::where('type', $fromType)->where('time_scale', $timeRange)->orwhere('type', $toType)->whereBetween(
            'time',
            array($fromTime, $toTime)
        )->delete();
    ...
    public function getTable(Request $request) {
        ...
        if ($exist) {
            ...
            $data = $this->getFromModel($type, $fromTime, $timeRange);
        } else {
            // Call Oanda API
    ...
    public function getFromModel($type, $fromTime, $timeRange) {
